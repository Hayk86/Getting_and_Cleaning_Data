...
subset_X <- X[,grep("mean|std", names(X))]
        #Filtering out measures which are not raw, but rather derived
        #Such as Jerk, Magnitude, Furier Transformation and Mean Frequency
...
#Reverting data back to the wide format when in each column there is
# a different measurement mean
